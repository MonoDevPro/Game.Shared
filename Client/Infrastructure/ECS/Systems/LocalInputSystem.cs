...
public partial class LocalInputSystem(World world) : BaseSystem<World, float>(world)
{
    ...
    private void ProcessInput(in Entity entity)
    {
        ...
        if (Godot.Input.IsActionPressed(GodotInputMap.MOVE_UP))    intent.Y = -1;
        if (Godot.Input.IsActionPressed(GodotInputMap.MOVE_DOWN))  intent.Y = 1;
        if (Godot.Input.IsActionPressed(GodotInputMap.MOVE_LEFT))  intent.X = -1;
        if (Godot.Input.IsActionPressed(GodotInputMap.MOVE_RIGHT)) intent.X = 1;
